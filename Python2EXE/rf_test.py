...
print rfmsgID
if rfmsgID == "01":
    tasterState = line[(dataIndex+ len("DATA:") + 2):(dataIndex+ len("DATA:") + 4)]
...
print rfmsgID
if rfmsgID == "02":
    dutSN = line[(dataIndex + len("DATA:") + len(rfmsgID)):(dataIndex + len("DATA:") + len(rfmsgID) + 16)]
    print "dutSN =  " + str(dutSN)
    
    dutSN = repackHex(dutSN)
    print "dutSN =  " + str(dutSN)
    
    devID = line[(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN)):(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + 2)]
    print "devID =  " + str(devID)
    
    hwVersion = line[(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + len(devID)):(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + len(devID) + 2)]
    print  "hwVersion =  " + str(hwVersion)
   
    stackVersion = line[(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + len(devID) + len(hwVersion)):(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + len(devID) + len(hwVersion) + 2)]
    print  "stackVersion =  " + str(stackVersion)
    
    appVersion = line[(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + len(devID) + len(hwVersion) + len(stackVersion)):(dataIndex + len("DATA:") + len(rfmsgID) + len(dutSN) + len(devID) + len(hwVersion) + len(stackVersion) + 2)]
